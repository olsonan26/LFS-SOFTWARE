...
export function StudentTimeline({ person, mode, year, month, onYearChange, onMonthChange }: Props) {
  ...
  const compoundRows: [string, CompoundValue | undefined][] = mode === 'ANNUAL'
  ...
  return <section className="student-timeline" aria-label={`${mode === 'ANNUAL' ? 'Annual' : 'Monthly'} student timeline`}>
    <div className="timeline-navigation">
      <div><h2>{mode === 'ANNUAL' ? 'Annual timeline' : 'Monthly timeline'}</h2><p>{person.displayName} · {dateLabel(focusYear,mode === 'MONTHLY' ? month : undefined)} · Age {focusAge}</p></div>
      ...
      {mode === 'MONTHLY' && <div className="timeline-month-controls">
    ...
    <div className="timeline-chart-card">
      <h3>{mode === 'ANNUAL' ? 'Yearly Timeline — Personal Cycles' : 'Yearly / Monthly Timeline Summary'}</h3>
      <div className="student-chart-paper">
        <div className="print-panel-scroll" tabIndex={0} role="region" aria-label="Timeline chart; scroll horizontally to explore">
          <TimelineAnnotations.Provider value={showPatterns ? (mode === 'ANNUAL' ? annualAnnotations(start,30) : monthAnnotations) : (mode === 'MONTHLY' ? {CM:monthAnnotations.CM} : {})}>
            {mode === 'ANNUAL' ? <PrintYearSection report={report} start={start} length={30} variant="focus"/> : <PrintMonthSection report={report} currentYear={currentYear} focusYear={focusYear}/>}
          </TimelineAnnotations.Provider>
        </div>
      </div>
      <p className="timeline-caption">{mode === 'ANNUAL' ? `${birthYear + start}–${birthYear + start + 29} · Ages ${start}–${start + 29} · * marks the current age.` : `${focusYear - 1}–${focusYear + 1} · January through December in each year. The gold frame marks the center year.`}</p>
    </div>
    {mode === 'ANNUAL' && <details className="timeline-extended" open><summary>Sequence Timeline — Extended Cycles · Ages {extendedStart}–{extendedStart+79}</summary>
    ...
    <section className="timeline-warning-panel" aria-label="Forensic pattern warnings">
      ...
      <div className="timeline-pattern-options">
        ...
        <label><input type="checkbox" checked={showDiagonals} onChange={e=>setShowDiagonals(e.target.checked)}/> {mode === 'ANNUAL' ? 'Birthday lapse' : 'End crossover'}</label>
        {mode === 'ANNUAL' && <label><input type="checkbox" checked={showIntensifiers} onChange={e=>setShowIntensifiers(e.target.checked)}/>Intensifications</label>}
      </div>
